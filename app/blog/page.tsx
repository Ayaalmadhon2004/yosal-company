...
export const metadata = {
  ...
  description: "اكتشف مقالات حصرية حول تطوير الأنظمة، الـ SaaS، وأسرار النمو التجاري في مدونة يوصل الرقمية.",
};
  ...
  return (
    <main className="min-h-screen bg-[#0a0d1d] pt-32 pb-20 text-right font-sans" dir="rtl">
      ...
      <section className="max-w-6xl mx-auto px-6 mb-20 flex flex-col md:flex-row items-center gap-10">
        <div className="flex-1 space-y-6 animate-in fade-in slide-in-from-right duration-700">
        ...
        {posts.length > 0 && (
          <div className="flex-1 relative w-full h-[350px] rounded-[2.5rem] overflow-hidden border border-white/5 group shadow-2xl">
            <Image 
        ...
        <div className="bg-[#12162b]/80 backdrop-blur-md p-4 rounded-[2rem] border border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 shadow-lg">
          ...
          <form action="/blog" className="relative w-full md:w-72">
            ...
            <input 
              ...
              className="w-full bg-[#0a0d1d] border border-white/10 rounded-full py-3 pr-12 pl-6 text-white text-xs outline-none focus:border-orange-500 transition-all placeholder:text-gray-600"
            />
          </form>
        </div>
      </section>
        ...
        {posts.length > 0 ? (
          posts.map((post: any) => (
            <Link 
              href={`/blog/${post.slug}`} 
              key={post.id}
              className="group bg-[#12162b] rounded-[2.5rem] overflow-hidden border border-white/5 hover:border-orange-500/40 transition-all duration-500 flex flex-col shadow-xl"
            >
              ...
              <div className="p-7 flex flex-col flex-grow">
                <div className="flex items-center gap-3 text-gray-500 text-[10px] mb-4">
                  <span className="flex items-center gap-1">
                    <Calendar className="w-3 h-3" /> {post.date || "اليوم"}
                  </span>
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3" /> 5 دقائق
                  </span>
                </div>
                <h2 className="text-xl font-bold text-white mb-3 group-hover:text-orange-500 transition-colors line-clamp-2 leading-tight">
                  {post.title}
                </h2>
                <p className="text-gray-400 text-sm line-clamp-3 mb-6 leading-relaxed opacity-80">
          ...
          <div className="col-span-full text-center py-20">
            <p className="text-gray-500 text-xl">لا توجد نتائج لمجال بحثك حالياً.</p>
          </div>
        )}
      </section>

      <section className="max-w-6xl mx-auto px-6 mt-24">
        <div className="bg-gradient-to-br from-[#12162b] to-[#0a0d1d] rounded-[3rem] p-12 text-center border border-white/5 relative shadow-2xl overflow-hidden group">
          <div className="relative z-10 space-y-6">
            ...
            <form className="flex flex-col md:flex-row gap-4 max-w-lg mx-auto pt-4">
              <input 
                ...
                className="flex-grow bg-[#0a0d1d] border border-white/10 rounded-2xl px-6 py-4 text-white outline-none focus:border-orange-500 transition-all text-sm"
                required
              />
              <button className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-10 py-4 rounded-2xl transition-all shadow-lg active:scale-95">
                اشترك الآن
              </button>
            </form>
          </div>
