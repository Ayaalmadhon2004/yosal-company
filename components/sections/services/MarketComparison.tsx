          ...
          >
            <div className="mb-6 flex justify-start">
              <div className="p-3 rounded-2xl bg-destructive/10 border border-destructive/20">
                <AlertTriangle className="text-destructive w-6 h-6" aria-hidden="true" />
              </div>
            </div>
            ...
            <div className="space-y-4">
              {comparison.challenges.map((item: string, index: number) => (
                <div key={`challenge-${index}`} className="flex items-center gap-3 text-muted-foreground/80 justify-start group/item">
                  <XCircle className="w-5 h-5 text-destructive/40 flex-shrink-0 group-hover/item:text-destructive transition-colors" aria-hidden="true" />
          ...
          >
            ...
            <div className="space-y-4 relative z-10">
              {comparison.solutions.map((item: string, index: number) => (
                <div key={`solution-${index}`} className="flex items-center gap-3 text-foreground justify-start group/sol">
                  <CheckCircle2 className="w-5 h-5 text-primary flex-shrink-0 group-hover/sol:scale-110 transition-transform" aria-hidden="true" />
